...
/** 设置识别引擎的参�?
 ...
 2. vad_bos：前端点检测；静音超时时间，即用户多长时间不说话则当做超时处理�?单位：ms�?engine指定iat识别默认值为5000�?其他情况默认值为 4000，范�?0-10000�? 3. vad_eos：后断点检测；后端点静音检测时间，即用户停止说话多长时间内即认为不再输入， 自动停止录音；单�?ms，sms 识别默认值为 1800，其他默认值为 700，范�?0-10000�? 4. sample_rate：采样率，目前支持的采样率设置有 16000 �?8000�? 5. asr_ptt：是否返回无标点符号文本�?默认�?1，当设置�?0 时，将返回无标点符号文本�? 6. asr_sch：是否需要进行语义处理，默认�?，即不进行语义识别，对于需要使用语义的应用，需要将asr_sch设为1�? 7. result_type：返回结果的数据格式，可设置为json，xml，plain，默认为json�? 8. grammarID：识别的语法 id，只针对 domain 设置为”asr”的应用�? 9. asr_audio_path：音频文件名；设置此参数后，将会自动保存识别的录音文件。路径为Documents/(指定�?。不设置或者设置为nil，则不保存音频�? 10. params：扩展参数，对于一些特殊的参数可在此设置，一般用于设置语义�? @param key 识别引擎参数
 @param value 参数对应的取�?
